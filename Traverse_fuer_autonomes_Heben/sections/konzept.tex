...
Das horizontale Sichtfeld wird mit dem Tangens des Halbwinkel berechnet, welches zusammen mit der Höhe von einem Meter multipliziert wird.
...
\subsection{Evaluation Konzept}
Um die Lokalisierung durch Machine-Learning durchzuführen braucht es genügend gelabelte Bilder, um ein Modell zu trainieren, da kein vortrainiertes Modell vorhanden ist. 
Das auftreiben und labeln von diesen Bildern würde Zeit und Kosten brauchen. 
Die Kunden bestätigten das Marker an der Last angebracht werden können und dass sichergestellt werden kann, dass diese den gleichen Abstand zueinander haben, weshalb die Lokalisierung mit Marker ausgewählt wurde.

Des Weiteren wird mit der Marker Anordnung die Robustheit sichergestellt vom System, so dass bei Wetterbedingungen, welche Marker zum Teil verdecken können, es trotzdem potenzielle andere Marker gibt.

Weiters sollte durch die berechneten Kamera Eigenschaften sichergestellt werden, dass die Genauigkeiten erfüllt werden können. 

Mit diesem Konzept können die Anforderungen alle erfüllt werden.